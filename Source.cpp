			...
			if (!crm.checkTourist(name)) {
				...
				if (command == 1) {
					...
					for (int i = 0; i < match.getSearches().size(); i++) {
						...
						for (int j = 0;j < match.getValids().size();j++) {
				...
				cout << "You are already booked with " << crm.tourguide(name) << endl;
		}
	}
	else if (choice == 2) {
		TourGuide x;
